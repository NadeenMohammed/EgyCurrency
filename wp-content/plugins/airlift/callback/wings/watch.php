	...
	public function process($request) {
		...
		case "getdata":
			...
			if (array_key_exists('lp', $params)) {
				...
				$lp_params = $params['lp'];
				if (!isset($lp_params['bv_check_table']) || $db->isTablePresent($db->getBVTable(ALProtectLP_V546::TABLE_NAME))) {
					$limit = intval($lp_params['limit']);
					$filter = $lp_params['filter'];
					$db->deleteBVTableContent(ALProtectLP_V546::TABLE_NAME, $lp_params['rmfilter']);
					$table = $db->getBVTable(ALProtectLP_V546::TABLE_NAME);
			...
			if (array_key_exists('fw', $params)) {
				...
				$fw_params = $params['fw'];
				if (!isset($fw_params['bv_check_table']) || $db->isTablePresent($db->getBVTable(ALProtectFW_V546::TABLE_NAME))) {
					$limit = intval($fw_params['limit']);
					$filter = $fw_params['filter'];
					$db->deleteBVTableContent(ALProtectFW_V546::TABLE_NAME, $fw_params['rmfilter']);
					$table = $db->getBVTable(ALProtectFW_V546::TABLE_NAME);
