...
class BVAccountCallback extends BVCallbackBase {
	...
	function process($request) {
		...
		case "addacc":
			ALAccount::addAccount($this->settings, $params['public'], $params['secret']);
			$resp = array("status" => ALAccount::exists($this->settings, $params['public']));
			break;
		case "rmacc":
			$resp = array("status" => ALAccount::remove($this->settings, $params['public']));
			break;
		case "updt":
			$account->updateInfo($params);
			$resp = array("status" => ALAccount::exists($this->settings, $params['pubkey']));
			break;
		case "updtapikey":
			ALAccount::updateApiPublicKey($this->settings, $params['pubkey']);
			$resp = array("status" => $this->settings->getOption(ALAccount::$api_public_key));
			break;
		case "rmbvscrt":
		...
		case "rmoldbvacc":
			...
			break;
		case "fetch":
			$accounts = ALAccount::allAccounts($this->settings);
