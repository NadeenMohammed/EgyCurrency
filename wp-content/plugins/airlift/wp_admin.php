...
class ALWPAdmin {
	...
	public function initHandler() {
		...
		if ($this->bvinfo->isActivateRedirectSet()) {
			...
			if ($apipubkey && isset($apipubkey)) {
				...
				wp_redirect($this->bvinfo->appUrl().'/plugin/activate?bvpublic=cfef9bb25512cfcae3a3df247a20672f&siteurl='.$siteurl.'&adminurl='.$adminurl);
				exit();
			}
	...
	public function bvsecAdminMenu($hook) {
		if ($hook === 'toplevel_page_airlift' || ALHelper::safePregMatch("/bv_add_account$/", $hook) ||
			...
			wp_enqueue_style( 'sosurface', plugins_url('css/bvmui.min.css', __FILE__));
...
<?php
	...
	public function createSoAdminMenu() {
		...
		$wp_admin_bar->add_menu(array('id' => $menu_id, 'title' => __('AirLift Options')));
