...
function alister_bank_theme_fonts_generate() {
	...
	foreach ($font_fields as $font_field) {
		$font_option = 'alister-bank' . '_' . $font_field . '_font_google_font';
...
function cmsmasters_theme_font($fonts, $font_name = '') {
	...
	if ( 
		...
		if ($web_fonts != '') {
			...
			if (
				isset($cmsmasters_option['alister-bank' . '_google_web_fonts_subset']) && 
				is_array($cmsmasters_option['alister-bank' . '_google_web_fonts_subset'])
			) {
				$web_fonts_subset_array = $cmsmasters_option['alister-bank' . '_google_web_fonts_subset'];
			} else {
...
function alister_bank_the_widgets_init() {
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_default', 
            'description' => esc_html__('Widgets in this area will be shown in all left and right sidebars till you don\'t use custom sidebar. To display the sidebar please choose the Right or Left Layout Type in the Theme Settings - General - Content options.', 'alister-bank'), 
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_bottom', 
            'description' => esc_html__('Widgets in this area will be shown at the bottom of middle block below the content and middle sidebar, but above footer. To display Bottom Sidebar please enable it in the Theme Settings - General - Content - Bottom Sidebar Visibility by Default. You can also display it on concrete page in the Page Layout options.', 'alister-bank'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_archive', 
            'description' => esc_html__('Widgets in this area will be shown in all left and right sidebars on archives pages. To display the sidebar please choose the Right or Left Layout Type in the Theme Settings - General - Content options.', 'alister-bank'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
    ...
    register_sidebar(
        array(
            ...
            'id' => 'sidebar_search', 
            'description' => esc_html__('Widgets in this area will be shown in left or right sidebar on search page. To display the sidebar please choose the Right or Left Layout Type in the Theme Settings - General - Content options.', 'alister-bank'), 
            'before_widget' => '<aside id="%1$s" class="widget %2$s">', 
            'after_widget' => '</aside>', 
            'before_title' => '<h3 class="widgettitle">', 
            'after_title' => '</h3>'
...
function cmsmasters_get_tweets( $username, $count ) {
	$backup_name = 'cmsmasters_' . 'alister-bank' . '_tweets_list_backup';

	$backup_tweets = get_option( $backup_name );

	if ( 'done' === get_transient( 'cmsmasters_' . 'alister-bank' . '_tweets_list_regeneration' ) && ! empty( $backup_tweets ) ) {
		return $backup_tweets;
	}

	$cmsmasters_option = alister_bank_get_global_options();

	if ( empty( $cmsmasters_option['alister-bank' . '_twitter_access_token'] ) ) {
		return $backup_tweets;
	}
	...
	$response = wp_remote_get(
		...
		array(
			'headers' => array(
				'Authorization' => 'Bearer ' . esc_html( $cmsmasters_option['alister-bank' . '_twitter_access_token'] ),
			),
			'body' => array(
	...
	set_transient( 'cmsmasters_' . 'alister-bank' . '_tweets_list_regeneration', 'done', HOUR_IN_SECONDS * 6 );
...
function alister_bank_logo() {
	...
	if ($cmsmasters_option['alister-bank' . '_logo_type'] == 'text') {
		if ($cmsmasters_option['alister-bank' . '_logo_title'] != '') {
			$blog_title = stripslashes($cmsmasters_option['alister-bank' . '_logo_title']);
		} else {
		...
		if ($cmsmasters_option['alister-bank' . '_logo_subtitle'] != '') {
			$blog_descr = stripslashes($cmsmasters_option['alister-bank' . '_logo_subtitle']);
		} else {
...
function cmsmasters_ajax_apply_theme_style_callback() {
	...
	if (wp_verify_nonce($nonce, 'cmsmasters_ajax_apply_theme_style-nonce')) {
		if (isset($_POST['theme_style'])) {
			...
			update_option('cmsmasters_alister-bank_theme_style_change', 'change');
		}
...
function alister_bank_theme_style_change() {
	if (!CMSMASTERS_THEME_STYLE_COMPATIBILITY) {
		update_option('cmsmasters_alister-bank_theme_style', '');

		return;
	}

	if ('hold' === get_option('cmsmasters_alister-bank_theme_style_change')) {
		return;
	} else {
		...
		update_option('cmsmasters_alister-bank_theme_style_change', 'hold');
	}
}
