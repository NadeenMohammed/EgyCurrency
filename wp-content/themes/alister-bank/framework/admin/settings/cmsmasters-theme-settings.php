...
function alister_bank_settings_scripts() {
	...
	wp_localize_script('alister-bank-settings-js', 'cmsmasters_setting', array( 
		...
		'remove_sidebar' => 				esc_html__('Do you really want to remove this sidebar?', 'alister-bank'), 
		'remove_google_web_font' => 		esc_html__('Do you really want to remove this google web font?', 'alister-bank'), 
		'find' => 							esc_attr__('Find icons', 'alister-bank'), 
		'remove_icon' => 					esc_html__('Do you really want to remove this social icon?', 'alister-bank'), 
...
function alister_bank_form_field_fn($args = array()) {
			...
			echo '<div class="cmsmasters_admin_block">' . 
				'<select class="select" id="' . $id . '_google_font" name="' . $cmsmasters_option_name . '[' . $id . '_google_font]">';
					...
					if (!empty($cmsmasters_fonts_list['local'])) {
						echo '<optgroup label="' . esc_attr__('Local Fonts', 'alister-bank') . '">';
					...
					if (!empty($cmsmasters_fonts_list['web'])) {
						echo '<optgroup label="' . esc_attr__('Google Web Fonts', 'alister-bank') . '">';
	...
	case 'google_web_fonts':
		...
		'<div class="google_web_fonts_manager">' . 
				...
				if (isset($options[$id]) && is_array($options[$id])) {
					...
					foreach($options[$id] as $font) {
						...
						echo '<div class="google_web_fonts_manager_item">' . 
							'<input class="all-options" type="text" id="google_web_font_family" placeholder="' . esc_attr__('Font Family (for example: Roboto Slab)', 'alister-bank') . '" name="' . $cmsmasters_option_name . '[' . $id . '_-_' . $i . '_family]" value="' . esc_attr($font[1]) . '" />' . 
							'<input class="regular-text" type="text" id="google_web_font_style" placeholder="' . esc_attr__('Font Style (for example: Roboto+Slab:100,300,400,700)', 'alister-bank') . '" name="' . $cmsmasters_option_name . '[' . $id . '_-_' . $i . '_style]" value="' . esc_attr($font[0]) . '" />' . 
			...
			'<input class="button" type="button" id="add_google_web_font" value="' . esc_attr__('Add Font', 'alister-bank') . '" data-font-family-label="' . esc_html__('Font Family (for example: Roboto Slab)', 'alister-bank') . '" data-font-style-label="' . esc_attr__('Font Style (for example: Roboto+Slab:100,300,400,700)', 'alister-bank') . '" data-option-name="' . $cmsmasters_option_name . '[' . $id . '_-_" />' . 
		'</div>';
...
function alister_bank_add_global_options($reset = false) {
	$cmsmasters_option_names = array( 
		...
		array( 
			'cmsmasters_options_' . 'alister-bank' . CMSMASTERS_THEME_STYLE . '_font_google', 
		...
		if ($cmsmasters_option_name[1] !== '') {
			foreach ($cmsmasters_option_name[1] as $selected_option) {
				if ( 
					...
					$selected_option['id'] !== 'alister-bank' . '_google_web_fonts' 
				) {
