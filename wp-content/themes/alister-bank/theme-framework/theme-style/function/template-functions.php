	...
	if (
		...
		$out_form = "<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<div class=\"cmsmasters_header_search_form_field\">
				...
				<button type=\"submit\">" . esc_html__('Search', 'alister-bank') . "</button>
			</div>
		</form>";
				...
				if ($cmsmasters_heading == 'custom') {
					...
					if (is_search()) {
						...
						if (!empty($wp_query->found_posts)) {
							echo sprintf(esc_html(_n('%1$d search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'alister-bank')), $wp_query->found_posts, get_search_query());
						} else {
