...
function alister_bank_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'alister-bank') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
		...
		if ($r_query->have_posts()) {
			echo "<aside class=\"cmsmasters_single_slider\">" . 
				"<" . esc_html($tag) . " class=\"cmsmasters_single_slider_title\">" . 
					($title != '' ? esc_html($title) : esc_html__('More items', 'alister-bank')) . 
				"</" . esc_html($tag) . ">" . 
...
function alister_bank_get_pings($id, $tag = 'h3') {
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'alister-bank') . '</' . esc_html($tag) . '>' . "\n" .
